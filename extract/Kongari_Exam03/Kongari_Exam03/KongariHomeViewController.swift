...
class KongariHomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            //create the cell
            let cell = KongariTVOL.dequeueReusableCell(withIdentifier: "kongariCell", for: indexPath)//indexPath is 0-4 indexes of array in this case
    ...
    @IBOutlet weak var KongariTVOL: UITableView!
    ...
    override func viewDidLoad() {
        ...
        KongariTVOL.dataSource = self
        KongariTVOL.delegate = self
        ...
        games.append(tableTennis)
        games.append(volleyball)
        games.append(chess)
        games.append(snooker)
        ...
        games.append(tennis)
        games.append(hockey)
        games.append(handball)
        games.append(golf)
        games.append(trackandField)
        games.append(cycling)
        games.append(rugby)
        games.append(surfing)
        
        games.sort{($0.name ?? "") < ($1.name ?? "")}
        KongariTVOL.reloadData()
        
    }
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        var transition = segue.identifier
        if transition == "kongariDescriptionSegue"{
            var destination = segue.destination as! KongariGameController
            destination.gameResult = games[(KongariTVOL.indexPathForSelectedRow?.row)!]
        }
    }
